...
!(async () => {
  ...
  let response = await Promise.allSettled([
    ...
    axios.get(`https://api.covid19api.com/country/Brazil?from=${new Date(2021, 04, 10, -3, 0, 0).toISOString()}&to=${new Date(2021, 04, 25, -3, 0, 0).toISOString()}`),
    axios.get(`https://api.covid19api.com/country/Brazil?from=${new Date(2021, 04, 09, -3, 0, 0).toISOString()}&to=${new Date(2021, 04, 24, -3, 0, 0).toISOString()}`
    ),
  ]);
...
function loadKPI(json) {
  document.getElementById("kpiconfirmed").innerHTML = _.last(json).Confirmed.toLocaleString("PT-BR");
  document.getElementById("kpideaths").innerHTML = _.last(json).Deaths.toLocaleString("PT-BR");
  document.getElementById("kpirecovered").innerHTML = _.last(json).Recovered.toLocaleString("PT-BR");
}
